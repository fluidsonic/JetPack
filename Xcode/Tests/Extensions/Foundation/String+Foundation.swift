...
class String_Tests: XCTestCase {
	...
	func testFirstMatchForRegularExpression() {
		...
		XCTAssertEqual("abcabc".firstMatchForRegularExpression(try! NSRegularExpression(pattern: "(b)(c)|(nope)", options: []))!, ["bc", "b", "c", nil])
	...
	func testUrlEncoding() {
		...
		XCTAssertEqual(url.urlEncodedPath,
		               "https://github.com/fluidsonic/JetPack/blob/master/Xcode/Tests/Extensions/Foundation/NSCharacterSet.swift%3Fa=b&c=d+e%23L1")
		...
		let invalidString = String(bytes: [0xD8, 0x00], encoding: String.Encoding.utf16BigEndian)!
